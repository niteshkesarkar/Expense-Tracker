...
?>
	<script type="text/javascript">
		function EditAttribute(att_id,pid)
		{
				...
				url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.EditAttribute&tmpl=component&att_id='+att_id+'&pid='+pid,
				type: 'GET',
				dataType: 'json',
		...
		function AddNewAttribute(pid,count)
		{
			...
			//alert(rowCount);
			techjoomla.jQuery.ajax({
			url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.AddNewAttribute&tmpl=component&pid='+pid+'&count='+rowCount,
			type: 'GET',
			dataType: 'json',
		...
		function deleteAttribute(dellink,pid)
		{
				if (confirm("<?php echo JText::_( 'COM_QUICK2CART_ATTRIBUTE_DELET_CONFIRM' ); ?>") == true)
				{
					...
					techjoomla.jQuery.ajax({
					url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.delattribute&tmpl=component&pid='+pid+'&attr_id='+dellink,
					type: 'GET',
					dataType: 'json',
