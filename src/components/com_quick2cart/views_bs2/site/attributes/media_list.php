...
<?php
	...
	<script type="text/javascript">
		...
		function EditFile(file_id,pid)
		{
				...
				url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.EditMediFile&tmpl=component&pid='+pid+'&file_id='+file_id,
				type: 'GET',
				dataType: 'json',
			...
			if (confirm("<?php echo JText::_( 'COM_QUICK2CART_EFILE_DELET_CONFIRM' ); ?>") == true)
				{
					var tr_id = '.file_'+file_id;
					techjoomla.jQuery.ajax({
					url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.deleteMediFile&tmpl=component&pid='+pid+'&file_id='+file_id,
					type: 'GET',
					dataType: 'json',
		...
		function AddNewMedia(pid,count)
		{
			...
			techjoomla.jQuery.ajax({
			url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.EditMediFile&tmpl=component&pid='+pid+'&count='+rowCount,
			type: 'GET',
			dataType: 'json',
	...
	<a rel="{handler: 'iframe', size: {x: window.innerWidth-350, y: window.innerHeight-150}, onClose: function(){AddNewMedia(<?php echo $fparam; ?>);}}" class="btn btn-primary btn-small <?php echo ($button_dis == ""?'modal':$button_dis)?> " href="<?php echo ($button_dis == ""?$addMediaLink:"#")?> qtcAddMediaLink" style="display: inline;">
